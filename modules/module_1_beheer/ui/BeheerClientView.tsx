...
// --- Nieuwe Type Definities die matchen met graph.ts ---
...
export function BeheerClientView({
  ...
  // Verzamel alle ID's van objecten in de keten (nu met flatMap omdat 'nodes' genest zit)
  const existingRelatedIds = graphData
    ? [
        ...graphData.incomingChain.flatMap((step) =>
          step.nodes.map((n) => n.object.id)
        ),
        ...graphData.outgoingChain.flatMap((step) =>
          step.nodes.map((n) => n.object.id)
        ),
      ]
    : [];
    ...
    <main className="max-w-4xl mx-auto px-6 py-8 flex-1 w-full space-y-6">
      ...
      {!graphData ? (
        ...
        <div className="space-y-6">
          {/* SECTION 1: INGAANDE OBJECTEN (OMHOOG) */}
          ...
          {/* SECTION 3: UITGAANDE OBJECTEN (OMLAAG) */}
          <section className="bg-slate-50 p-4 rounded-lg border">
            <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">
              ...
              <div className="flex flex-col items-center space-y-4 w-full">
                {graphData.outgoingChain.map((step, stepIndex) => (
                  ...
                  >
                    ...
                    {/* CONTAINER DIE FLEX-ROW WORDT BIJ SPLITSING */}
                    <div
                      className={`flex w-full ${
                        step.isBranch
                          ? "flex-row flex-wrap justify-center gap-4"
                    ...
                    >
                      {step.nodes.map((node) => (
                        ...
                        >
                          ...
                          {/* Alleen details tonen als dit de laatste laag is die we laten zien */}
